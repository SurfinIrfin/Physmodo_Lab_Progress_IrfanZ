...
%}
 A = importdata('PilotTesting03.trc','\t',5);
...
% 2) C7 = Seventh cervical vertebra (base of neck) = c7
for vX = 84
    c7XB = A.data(a,vX);
    c7XL = A.data(b,vX);
    c7XF = A.data(c,vX);
...
trunk_begin_x = c7XB - sacXB;
trunk_begin_y = c7YB - sacYB;
trunk_begin_z = c7ZB - sacZB;

trunk_vec_begin = [0, trunk_begin_y, trunk_begin_z];

% trunk vector at low of squat
trunk_low_x = c7XL - sacXL;
trunk_low_y = c7YL - sacYL;
trunk_low_z = c7ZL - sacZL;

trunk_vec_low = [0, trunk_low_y, trunk_low_z];

% trunk vector at finish of squat
trunk_finish_x = c7XF - sacXF;
trunk_finish_y = c7YF - sacYF;
trunk_finish_z = c7ZF - sacZF;

trunk_vec_finish = [0, trunk_finish_y, trunk_finish_z];
...
tibiaL_vec_begin = [0, L_tibia_begin_y, L_tibia_begin_z];
...
tibiaL_vec_low = [0, L_tibia_low_y, L_tibia_low_z];
...
tibiaL_vec_finish = [0, L_tibia_finish_y, L_tibia_finish_z];
...
tibiaR_vec_begin = [0, R_tibia_begin_y, R_tibia_begin_z];
...
tibiaR_vec_low = [0, R_tibia_low_y, R_tibia_low_z];
...
tibiaR_vec_finish = [0, R_tibia_finish_y, R_tibia_finish_z];
...
%---RIGHT---
% right wrist beginning x,y,z coordianates
